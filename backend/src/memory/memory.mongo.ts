// ✅ FILE: src/memory/memory.mongo.ts
...
// ⬇️ SOLO si necesitas listar pedidos desde aquí.
// Si no la usas, borra estas dos líneas y la función obtenerTodosLosPedidos.
import PedidoModel, { type IPedido } from '../models/Pedido.model.js'
...
export async function crearNuevoUsuario({
  ...
  await UserMemoryModel.create({
    ...
    name: nombre,
    apellido,
    correo: correo?.toLowerCase?.() ?? correo,
    firstSeen: Date.now(),
    lastSeen: Date.now(),
...
/** Usuarios con pagos pendientes (para dashboard admin) */
export async function obtenerUsuariosConPagoPendiente(): Promise<UserMemoryDoc[]> {
  return await UserMemoryModel.find({ estadoPago: 'pendiente' }).sort({ updatedAt: -1 }).lean().exec()
}

/** Usuarios con pago verificado */
export async function obtenerUsuariosConPagoVerificado(): Promise<UserMemoryDoc[]> {
  return await UserMemoryModel.find({ estadoPago: 'verificado' }).sort({ updatedAt: -1 }).lean().exec()
}

/** (Opcional) Listar todos los pedidos desde aquí */
// ❗️Si no se usa en ningún lado, elimina esta función para no depender del modelo.
export async function obtenerTodosLosPedidos(): Promise<IPedido[]> {
  return await PedidoModel.find({}).lean().exec()
}
